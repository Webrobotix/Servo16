...
Use at your own risk. The user is solely responsible for ensuring safe operation of hardware connected 
to this software. Not intended for children. The author assumes no responsibility for injuries, damages, 
 ...
 * Support for Arduino 16-Channel 12-bit PWM Servo Shield
 * Unified settings storage - no EEPROM, all data in files
 * REMOVED: Save button - all settings now saved together
...
// New colors for sequence recording
...
// NEW: PWM Shield colors
final color PWM_SHIELD_BUTTON_COLOR = color(255);

// Inactive/grayed out colors
...
// NEW: Sequence recording buttons
Button recordButton;
Button playButton;
...
// NEW: PWM Shield support
Button pwmShieldToggleButton;
boolean usePWMShield = false;
...
// NEW: Sequence recording variables
...
// NEW: Keyframe timing controls
Slider speedSlider;
Slider delaySlider;

// NEW: Sequence export variables
...
void drawSketchNamingDialog() {
  ...
  text(usePWMShield ? "☑ Arduino 16-Channel 12-bit PWM Servo Shield" : "☐ Standard Arduino Servo Library", width/2 + 100, height/2 + 5);
...
void mousePressed() {
  ...
  // NEW: Handle pagination button clicks in file selection
  if (isSelectingFile) {
  ...
  if (isEditingLabel) {
    ...
    if (key == ENTER) {
      if (isValidFileName(tempFileName)) {
        ...
        settingsStatus = "Invalid file name. Use only letters, numbers, hyphens, and underscores.";
    ...
    if (key == ENTER && selectedFileIndex >= 0) {
      ...
      selectedFileIndex = -1;
      fileOperation = "";
      fileListPage = 0; // NEW: Reset to first page
    } else if (key == ESC) {
      isSelectingFile = false;
      selectedFileIndex = -1;
      fileOperation = "";
...
void togglePWMShield() {
  ...
  sequenceStatus = "PWM Shield mode: " + (usePWMShield ? "ENABLED" : "DISABLED");
  sequenceStatusTime = millis();
}
...
void toggleRecording() {
  ...
  if (isRecording) {
    sequenceStatus = "Recording started - move servos and press K to add keyframes";
  } else {
...
void addKeyframe() {
  ...
  sequenceStatus = "Added keyframe " + sequence.size() + " with " + activeCount + " servos";
  sequenceStatusTime = millis();
}
...
void playSequence() {
  ...
  if (isPlayingSequence) {
    ...
    sequenceStatus = "Sequence playback stopped";
  } else {
...
void checkForNewSerialPorts() {
  ...
  previousSerialPorts = currentPorts;
...
void establishConnection() {
  ...
  connectionStatus = "Scanning " + serialPorts.length + " ports...";
  connectionStatusTime = millis();
...
void saveSettingsToFileWithName(String fileName) {
  ...
  settingsData[0] = "# Servo Settings and Labels: " + fileName;
  settingsData[1] = "# Saved on: " + day() + "/" + month() + "/" + year();
  settingsData[2] = "# Settings Format: servoNum:minLimit:maxLimit:centerPos:active";
  settingsData[3] = "# Labels Format: L:servoNum:labelText";
  ...
  settingsStatus = "Settings saved as '" + fileName + "'";
  settingsStatusTime = millis();
}
...
void loadSettingsFromFileWithName(String fileName) {
  try {
    ...
    if (settingsData == null || settingsData.length <= 4) {
      settingsStatus = "Could not read file '" + fileName + "'";
      settingsStatusTime = millis();
      return;
    }
...
void deleteSettingsFileWithName(String fileName) {
  try {
    ...
    if (settingsFile.exists() && settingsFile.delete()) {
      settingsStatus = "File '" + fileName + "' deleted";
      settingsStatusTime = millis();
    } else {
      settingsStatus = "Failed to delete '" + fileName + "'";
      settingsStatusTime = millis();
    }
  } catch (Exception e) {
    settingsStatus = "Error deleting '" + fileName + "'";
    settingsStatusTime = millis();
  }
}
...
void loadSettingsFromFile() {
  ...
  if (settingsStatus.contains("Could not read") || settingsStatus.contains("Error")) {
    settingsStatus = "No default settings - using defaults";
    settingsStatusTime = millis();
  }
}
...
void createUI() {
  ...
  saveSettingsButton = new Button(WINDOW_WIDTH - 700, WINDOW_HEIGHT - 50, 100, 40, "Save Settings", SAVE_SETTINGS_BUTTON_COLOR);
  loadSettingsButton = new Button(WINDOW_WIDTH - 590, WINDOW_HEIGHT - 50, 100, 40, "Load Settings", LOAD_SETTINGS_BUTTON_COLOR);
  deleteSettingsButton = new Button(WINDOW_WIDTH - 480, WINDOW_HEIGHT - 50, 110, 40, "Delete Settings", DELETE_SETTINGS_BUTTON_COLOR);
...
void drawServoControl(int servoNum) {
  ...
  text("Min: " + servoMinLimits[servoNum] + "° | Max: " + servoMaxLimits[servoNum] + 
       "° | Center: " + servoCenterPositions[servoNum] + "°", x + 135, y + 90);
...
void drawLabelEditDialog() {
  ...
  text("New Label: " + tempLabel + "_", width/2, height/2 + 10);
...
void drawFileNamingDialog() {
  ...
  text("(Use only letters, numbers, hyphens, underscores)", width/2, height/2 + 40);
}

// NEW: Updated file selection dialog with pagination
void drawFileSelectionDialog() {
  ...
  if (savedFilesList.length == 0) {
    text("No saved settings files found", width/2, height/2 - 50);
    textSize(14);
    text("Save some settings first", width/2, height/2 - 20);
  } else {
    int totalPages = (savedFilesList.length + FILES_PER_PAGE - 1) / FILES_PER_PAGE;
    int startIndex = fileListPage * FILES_PER_PAGE;
    int endIndex = min(startIndex + FILES_PER_PAGE, savedFilesList.length);
    ...
    textSize(14);
    text("Select a file (Total: " + savedFilesList.length + " | Page " + (fileListPage + 1) + 
         " of " + totalPages + "):", width/2, height/2 - 140);
    ...
    textAlign(CENTER, CENTER);
    textSize(14);
    text("Click to select, ENTER to " + operation.toLowerCase() + ", ESC to cancel", 
         width/2, height/2 + 90);
    
    if (selectedFileIndex >= 0 && selectedFileIndex < savedFilesList.length) {
      fill(0, 100, 200);
      textSize(12);
    ...
    fill(0);
    textSize(12);
    text("Showing " + (startIndex + 1) + "-" + endIndex + " of " + savedFilesList.length, 
